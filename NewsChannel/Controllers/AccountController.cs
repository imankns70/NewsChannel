...
using NewsChannel.ViewModel.Account;
...
namespace NewsChannel.Controllers
{
    public class AccountController : Controller
    {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    if (result.Succeeded)
                    {
                        ...
                        string callbackUrl = Url.Action("ConfirmEmail", "Account", values: new { userId = user.Id, code = code }, protocol: Request.Scheme);
        ...
        [HttpGet, AjaxOnly]
        public IActionResult DeleteBookmark(int? userId, int? newsId)
        {
            if (!userId.HasValue || !newsId.HasValue)
            {
